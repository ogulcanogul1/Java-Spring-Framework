...
@NamedNativeQuery(
    name = "Contact.findAllOpen",
    query = "SELECT * FROM contact_msg WHERE status = :status",
    resultClass = Contact.class
)
-------------------------------------------------------------------------------------Çok parametre dönen SqlResultMapping

@SqlResultSetMappings({
    @SqlResultSetMapping(
        name = "SqlResultSetMapping.contactMsg",
        columns = {
...
JPQL kullanımında mapping işlemi gerekmez. Sadece SQL kullanımında mapping işlemi yapılır.

Native SQL kullandığında Spring Data / JPA, dönen veriyi otomatik olarak Entity'ye nasıl eşleştireceğini anlayamaz.
*/
